                              ...
                              unsigned int const vertical_split_count)
{
	...
	glBufferData(GL_ARRAY_BUFFER, /*! \todo how many bytes should the buffer contain? */ 4 * 3 * sizeof(GLfloat),
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::vertices),
	                      ...
	                      /* once all components of a vertex have been read, how far away (in bytes) is the next vertex? */ 3 * sizeof(GLfloat),
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, /*! \todo how many bytes should the buffer contain? */ 2 * 3 * sizeof(GLuint),
                               ...
                               unsigned int const minor_split_count)
{
	...
	auto const major_axis_slice_edges_count = major_split_count + 1u;
	auto const minor_axis_slice_edges_count = minor_split_count + 1u;
	auto const minor_axis_slice_vertices_count = minor_axis_slice_edges_count + 1u;
	auto const major_axis_slice_vertices_count = major_axis_slice_edges_count + 1u;
	auto const vertices_nb = minor_axis_slice_vertices_count * major_axis_slice_vertices_count;
	...
	float const d_theta = glm::two_pi<float>() / (static_cast<float>(major_axis_slice_edges_count));
	float const d_phi = glm::two_pi<float>() / (static_cast<float>(minor_axis_slice_edges_count));

	size_t index = 0u;
	float theta = 0.0f;
	float phi = 0.0f;
	float const ra = major_radius;
	float const rb = minor_radius;
	for (unsigned int i = 0u; i < major_axis_slice_vertices_count; ++i) {
		float const cos_theta = std::cos(theta);
		float const sin_theta = std::sin(theta);

		phi = 0.0f;
		for (unsigned int j = 0u; j < minor_axis_slice_vertices_count; ++j) {
			float const cos_phi = std::cos(phi);
			float const sin_phi = std::sin(phi);
			...
			// texture coordinates
			texcoords[index] = glm::vec3(static_cast<float>(j) / (static_cast<float>(minor_axis_slice_vertices_count)),
				static_cast<float>(i) / (static_cast<float>(major_axis_slice_vertices_count)),
				0.0f);

			// tangent
			auto const b = glm::vec3(-rb * sin_theta * cos_phi, -rb * cos_theta, -rb * sin_theta * sin_phi);

			tangents[index] = b;

			// binormal
			auto const t = glm::vec3(-(ra + rb * cos_theta) * sin_phi, 0, (ra + rb * cos_theta) * cos_phi);
			binormals[index] = t;
	...
	// create index array
	auto index_sets = std::vector<glm::uvec3>(2u * major_axis_slice_edges_count * minor_axis_slice_edges_count);

	// generate indices iteratively
	index = 0u;
	for (unsigned int i = 0u; i < major_axis_slice_edges_count; ++i)
	{
		for (unsigned int j = 0u; j < minor_axis_slice_edges_count; ++j)
		{
			index_sets[index] = glm::uvec3(minor_axis_slice_vertices_count * (i + 0u) + (j + 0u),
				minor_axis_slice_vertices_count * (i + 0u) + (j + 1u),
				minor_axis_slice_vertices_count * (i + 1u) + (j + 1u));
			++index;

			index_sets[index] = glm::uvec3(minor_axis_slice_vertices_count * (i + 0u) + (j + 0u),
				minor_axis_slice_vertices_count * (i + 1u) + (j + 1u),
				minor_axis_slice_vertices_count * (i + 1u) + (j + 0u));
			++index;
		}
	}
